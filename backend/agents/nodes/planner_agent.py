...
from backend.src.llm.ollama_client import OllamaModel
from backend.src.llm.prompt_builder import build_planner_prompt
...
def planner_agent_node(state):
    ...
    try:
        ...
        top_correlations = sorted(
            correlations, key=lambda x: abs(x["value"]), reverse=True
        )[:3]

        # Sort biggest anomalies first
        top_anomalies = sorted(
            anomalies, key=lambda x: abs(x["z_score"]), reverse=True
        )[:3]

        # Keep only strongest engine insights
        top_insights = metrics.get("insights", [])[:3]
        ...
        prompt = build_planner_prompt(planner_metrics, query)
